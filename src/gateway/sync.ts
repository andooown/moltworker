...
export async function syncToR2(sandbox: Sandbox, env: MoltbotEnv): Promise<SyncResult> {
  ...
  // Sync order: clawdbot config -> skills -> workspace (memory, agent files) -> auth profiles -> timestamp
  const syncCmd = `rsync -r --no-times --delete --exclude='*.lock' --exclude='*.log' --exclude='*.tmp' /root/.clawdbot/ ${R2_MOUNT_PATH}/clawdbot/ && rsync -r --no-times --delete /root/clawd/skills/ ${R2_MOUNT_PATH}/skills/ && rsync -r --no-times --delete --exclude='skills/' --exclude='*.lock' --exclude='*.log' --exclude='*.tmp' /root/clawd/ ${R2_MOUNT_PATH}/workspace/ && mkdir -p ${R2_MOUNT_PATH}/auth && if [ -f /root/.clawdbot/agents/main/agent/auth-profiles.json ]; then cp /root/.clawdbot/agents/main/agent/auth-profiles.json ${R2_MOUNT_PATH}/auth/auth-profiles.json; fi && date -Iseconds > ${R2_MOUNT_PATH}/.last-sync`;
