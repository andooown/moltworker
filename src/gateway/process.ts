 ...
 *
 * This will:
 * 1. Configure rclone for R2 persistence
...
export async function ensureMoltbotGateway(sandbox: Sandbox, env: MoltbotEnv): Promise<Process> {
  ...
  try {
    ...
    try {
      ...
      if (logs.stdout) console.log('[Gateway] startup logs:', logs.stdout);
    } catch {
    ...
    try {
      ...
      const errorMsg = `OpenClaw gateway failed to start. Stderr: ${logs.stderr || '(empty)'}`;
      markStartupFailed(errorMsg);
      throw new Error(errorMsg, { cause: e });
    } catch (logErr) {
      if (logErr instanceof Error && logErr.cause === e) throw logErr;
      console.error('[Gateway] Failed to get logs:', logErr);
      markStartupFailed(e instanceof Error ? e.message : String(e));
      throw e;
    }
  }
